...
def add_note(title: str, content: str) -> str:
    """Saves a detailed note for Dr. Aakash."""
    try:
        key = db.key('Note')
...
def workspace_instruction(ctx):
    ...
    return f"""
You are the Workspace Executive Assistant for Dr. Aakash.
...
async def chat(request: UserRequest):
    try:
        final_reply = ""
        # FIX: Convert the Pydantic model to a dict explicitly
        # This prevents the 'model_copy' attribute error during internal ADK processing
        agent_input = {"user_input": str(request.prompt)} 

        async for event in root_agent.run_async(agent_input):
            if hasattr(event, 'text') and event.text:
                final_reply = event.text
